...
func NewParser(symbolLength int, tf string) (p Parser) {
	...
	if tf == "EU" {
		...
		if tf == "NZ" {
			p.channels = []int{
				// Thanks to Ian Rees for testing the NZ frequencies
				902419338, 902921088, 903422839, 903924589, 904426340, 904928090, // NZ freq (to be detected)
...
func (h Hop) String() string {
	return fmt.Sprintf("{ChannelIdx:%d ChannelFreq:%d FreqError:%d Transmitter:%d}",
		h.ChannelIdx, h.ChannelFreq, h.FreqError, h.Transmitter,
	)
}
...
func (p *Parser) SetHop(n int, tr int) Hop {
	...
	p.channel = p.hopPattern[p.hopIdx]
	p.freqError = p.freqerrTrChSum[tr][p.channel] / 10
	p.transmitter = tr
	return p.hop()
}
